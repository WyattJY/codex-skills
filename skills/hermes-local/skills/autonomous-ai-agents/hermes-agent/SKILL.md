...
**Weixin / WeChat setup from an agent session:** `hermes gateway setup` uses a curses-style platform picker that can be awkward to drive over PTY. When the user specifically asks to connect Weixin, prefer invoking the Weixin setup flow directly from the repo venv:

```bash
source venv/bin/activate  # or .venv/bin/activate
python -c "from hermes_cli.gateway import _setup_weixin; _setup_weixin()"
```

If only QR login + conservative defaults are needed, call `gateway.platforms.weixin.qr_login()`, then save `WEIXIN_ACCOUNT_ID`, `WEIXIN_TOKEN`, `WEIXIN_BASE_URL`, `WEIXIN_CDN_BASE_URL`, `WEIXIN_DM_POLICY=pairing`, and `WEIXIN_GROUP_POLICY=disabled` via `hermes_cli.config.save_env_value()`. Run it in PTY/background mode so the terminal QR code is visible. If the installed venv lacks `pip`, bootstrap with `python -m ensurepip --upgrade` before installing optional QR rendering support (`qrcode`).
...
**WyattJY local workflow implementation:** when configuring Hermes from Wyatt's multi-agent / local-memory implementation plan, use `references/wyatt-local-workflow-implementation.md`. It covers the small-profile architecture, external-drive memory layout, P0 skills, SQLite/FTS schema, secret hygiene, and verification checklist.
...
- **Weixin / personal WeChat setup**: QR login uses the iLink bot identity and can be configured through `hermes gateway setup`; when driving setup from an agent/tool session, see `references/weixin-gateway-qr-setup.md` for the direct `qr_login()` helper flow, env vars, allowlist defaults, verification commands, and common iLink pitfalls.
