...
function presentationsScratchRoots() {
  return Array.from(
    new Set([
      path.resolve(process.env.TMPDIR || "/tmp", "codex-presentations"),
      path.resolve("/tmp", "codex-presentations"),
    ]),
  );
}
...
function assertSafeWorkspace(workspace) {
  ...
  const scratchRoots = presentationsScratchRoots();
  if (scratchRoots.some((scratchRoot) => isWithin(resolvedWorkspace, scratchRoot))) {
    return;
  }

  const segments = pathSegments(resolvedWorkspace);
  const threadId = process.env.CODEX_THREAD_ID;
  if (threadId && segments.includes(threadId)) {
    return;
  }
  if (segments.some((segment) => /^manual-[A-Za-z0-9._-]+$/.test(segment))) {
    return;
  }
  ...
  throw new Error(
    [
      ...
      `Expected a path under ${scratchRoots.join(" or ")}, or a path containing CODEX_THREAD_ID/manual-* as a path segment.`,
    ].join("\n"),
  );
}
