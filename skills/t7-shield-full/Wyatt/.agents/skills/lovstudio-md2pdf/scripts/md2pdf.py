...
DEBUG_MD2PDF = os.environ.get("MD2PDF_DEBUG") == "1"


def _debug(message):
    if DEBUG_MD2PDF:
        print(f"[md2pdf-debug] {message}", flush=True)
...
class PDFBuilder:
    ...
    def parse_md(self, md):
        ...
        while i < len(lines):
            if DEBUG_MD2PDF:
                preview = lines[i][:120].replace("\t", " ")
                _debug(f"parse line {i+1}/{len(lines)}: {preview}")
            ...
            if re.match(r'^#{4,}\s+', stripped):
                title = re.sub(r'^#{4,}\s+', '', stripped).strip()
                if title:
            ...
            if plines:
                ...
                for pl in plines[1:]:
                    ...
                    if merged and pl and _is_cjk(merged[-1]) and _is_cjk(pl[0]):
                        ...
                        merged += ' ' + pl
                if DEBUG_MD2PDF and (len(merged) > 2000 or '![' in merged or '$$' in merged):
                    _debug(f"paragraph len={len(merged)}")
                story.append(Paragraph(md_inline(merged, ah), ST['body']))
                continue

            # Avoid parser stalls on unknown one-line constructs.
            i += 1
            continue
