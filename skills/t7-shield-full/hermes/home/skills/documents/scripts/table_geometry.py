...
def audit_docx_tables(path: Path) -> int:
    """Print table width/grid/cell-width checks. Returns issue count."""
    ...
    for table_idx, tbl in enumerate(root.findall(".//w:tbl", ns), start=1):
        ...
        print(f"table {table_idx}: tblW={width_type}:{width} grid_sum={grid_sum} grid={grid}")
    ...
    if issues == 0:
        print("OK: all tables have matching tblW, tblGrid, and tcW")
    else:
